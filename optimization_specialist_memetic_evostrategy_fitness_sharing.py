...
def calculate_genotypic_diversity_top_10(population, fitness_values):
    ...
    top_10_indices = sorted_indices[:10]  # Get the indices of the top 10 individuals
    ...
    for i in range(10):
        for j in range(i + 1, 10):
            total_distance += np.linalg.norm(top_10_population[i] - top_10_population[j])  # Euclidean distance
            num_pairs += 1
