...
def parallel_island_run(env, world_population, pop_fit, step_sizes, mutation_rate):
    ...
    for i in range(n_islands):
        # Evolve the population on the island
        ...
        # Find the index of the best individual on this island based on fitness
        best_individual_idx = np.argmax(pop_fit[i])

        # Extract the best individual (a single array of size (265,))
        best_individual = world_population[i][best_individual_idx]

        # Add the best individual to the list
        best_individuals.append(best_individual)

    # Return only the best individuals, each of size (265,)
...
def test_solution_against_all_enemies(winner):
    ...
    controller = memetic_controller.player_controller(n_hidden=10)  # Create controller instance

    env = setup_environment("testing_against_all8", controller, enemies=all_enemies)
    env.update_parameter('speed', 'fastest')
    env.update_parameter("visuals", False)

    # Set the controller with the weights
    controller.set(winner, env.get_num_sensors())  # Set the weights (winner) into the controller

    # Run the game and get the fitness score
    total_gains = []
    total_fitnesses = []
    for i in range(10):
        total_fitness, player_life, enemy_life, _ = env.play(pcont=winner)  # Pass the weights (winner)
