...
def simulation(env, x, enemies):
    # Set the environment to use all enemies at once
    env.update_parameter('enemies', enemies)  # Pass all enemies at once
    env.update_parameter('multiplemode', 'yes')  # Ensure we are in "multiple mode"
    
    # Simulate and get the fitness
    f, p, e, t = env.play(pcont=x)
    
    return f  # Return the fitness for the individual across multiple enemies
...
def evaluate_fitnesses(env, population, enemies):
    def run_game_with_enemy(individual):
        return simulation(env, individual, enemies)  # No need to pass enemies here again, simulation handles it

    fitnesses = []
    for ind in population:
        # Parallelize the fitness evaluation for each individual (simulation handles enemies)
        fitness = run_game_with_enemy(ind)  # Call simulation directly
...
def init_population(pop_size, env, n_vars, dom_l, dom_u):
    ...
    fit_pop = evaluate_fitnesses(env, pop, enemies= DEFAULT_ENEMY)
...
def hill_climb(env, individual, mutation_rate, n_iterations=LOCAL_SEARCH_ITER):
    ...
    best_fitness = simulation(env, best_individual,enemies=DEFAULT_ENEMY)
    for _ in range(n_iterations):
        new_individual = individual + mutation_rate * np.random.randn(*individual.shape)
        new_fitness = simulation(env, new_individual,enemies=DEFAULT_ENEMY)
...
def main():
    ...
    # Set up environment
    experiment_name = "multi_demo_memetic_[4,6,8]"
    if not os.path.exists(experiment_name):
        os.makedirs(experiment_name)
    ...
    if os.path.exists(experiment_name + '/memetic_population_evo_strategy.pkl'):
        # Continue evolution
        ...
        fit_pop = evaluate_fitnesses(env, pop, DEFAULT_ENEMY)

        ini_g = 0

    # Run the Memetic Algorithm with DE
    print("\nRunning the Memetic Algorithm with Evo_Strategy...")
