...
def calculate_genotypic_diversity_top_10(population, fitness_values):
    """
...
def memetic_algorithm(env, pop, raw_fit_pop, shared_fit_pop, npop, gens, ini_g, n_vars, mutation_rate, sigma_share, experiment_name):
    ...
    for generation in range(ini_g, gens):
        ...
        # diversity
        genotypic_diversity = calculate_genotypic_diversity_top_10(pop, raw_fit_pop)
...
def main(experiment_name, enemy=DEFAULT_ENEMY, iteration_number=1):
    # Parameters
    npop = DEFAULT_POP_SIZE
    gens = DEFAULT_GENS
    mutation_rate = DEFAULT_MUTATION_RATE
    n_hidden_neurons = DEFAULT_HIDDEN_NEURONS
    dom_l, dom_u = -1, 1

    # Modify the experiment name for each run
    experiment_name = f"{experiment_name}_enemy{enemy}_run{iteration_number}"

    # Set up environment for this run
    env = setup_environment(experiment_name, enemy, player_controller(n_hidden_neurons))
    ...
    if os.path.exists(experiment_name + '/memetic_population.pkl'):
        print("\nContinuing evolution from saved population...\n")
        pop, fit_pop, ini_g = load_population_state(experiment_name)
    else:
        print("\nStarting new evolution...\n")
    ...
    # Save final solution for this run
    save_final_solution(experiment_name, best_solution, best_fitness)

    # Save population state for this run
    ...
    print(f"Simulation {iteration_number} completed.\n")


# Function to run multiple simulations
def run_multiple_simulations(base_experiment_name, enemy=DEFAULT_ENEMY, num_runs=10):
    for iteration in range(1, num_runs + 1):
        print(f"Running iteration {iteration}/{num_runs}...")
        main(base_experiment_name, enemy, iteration)


if __name__ == '__main__':
    # Example usage to run the simulation 10 times
    base_experiment_name = "memetic_optimization"
    run_multiple_simulations(base_experiment_name, num_runs=10)
